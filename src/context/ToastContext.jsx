...
function ToastItem({ toast, onRemove }) {
  return (
    <div style={{
      ...
      {ICONS[toast.type]}
      <p style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.82)', flex: 1, lineHeight: 1.4 }}>
        {toast.message}
      </p>
      <button
...
export function ToastProvider({ children }) {
  ...
  // Clear ALL pending timers when provider unmounts (memory leak fix)
  useEffect(() => {
    return () => {
